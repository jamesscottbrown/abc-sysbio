...
class GillespieCUDAWriter(Writer):
    ...
    def write(self):
        ...
        for i in range(len(self.parsedModel.stoichiometricMatrix[0])):
            for j in range(len(self.parsedModel.stoichiometricMatrix)):
                self.out_file.write("    " + repr(self.parsedModel.stoichiometricMatrix[j][i]))
                if not (i == (len(self.parsedModel.stoichiometricMatrix) - 1) and (
                    j == (len(self.parsedModel.stoichiometricMatrix[0]) - 1))):
                    self.out_file.write(",")
            self.out_file.write("\n")
