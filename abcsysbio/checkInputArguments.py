    ...
    if not source is None:
        ...
        for mod in range(0, len(source)):
            ...
            if not len(priors[mod]) == numParameters:
                return False, "\nThe number of given prior distributions for model " + ModelName[
                    mod] + " is not correct!\n"
        ...
        for ic in range(len(x0priors[mod])):
            if not (x0priors[mod][ic][0] == 0 or x0priors[mod][ic][0] == 1 or x0priors[mod][ic][0] == 2 or
                ...
                return False, "\nThe prior distribution of initial condition " + repr(ic + 1) + " in model " + \
                       ModelName[mod] + " does not exist!\n"

        for param in range(0, len(priors[mod])):
            if not len(priors[mod][param]) == 3:
                return False, "\nThe prior distribution of parameter " + repr(param + 1) + " in model " + ModelName[
            ...
            if priors[mod][param][0] == 2:
                if not priors[mod][param][1] < priors[mod][param][2]:
                    return False, "\nThe range of the uniform prior distribution of parameter " + repr(
                        param + 1) + " in model " + ModelName[mod] + " is wrong defined!\n"
