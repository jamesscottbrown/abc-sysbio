...
def modelfunction((m1,p1,m2,p2,m3,p3,),t,parameter=(1,2,5,1000,)):
	...
	if p3 > 0 : d_m1=-m1+alpha/(1 + pow(p3,n))+alpha0
	else : d_m1=-m1+alpha+alpha0

	d_p1=-beta*(p1-m1)

	if p1 > 0 : d_m2=-m2+alpha/(1 + pow(p1,n))+alpha0
	else : d_m2=-m2+alpha+alpha0

	d_p2=-beta*(p2-m2)

	if p2 > 0: d_m3=-m3+alpha/(1 + pow(p2,n))+alpha0
	else : d_m3=-m3+alpha+alpha0
